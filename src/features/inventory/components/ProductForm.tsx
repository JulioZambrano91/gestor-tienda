...
export function ProductForm({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) {
  ...
  const { bcvRate, convertToBs } = useBcv()
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-1 relative">
          <label className="text-sm font-medium flex justify-between text-slate-700 dark:text-slate-300">
            <span>Costo (USD)</span>
            <span className="text-slate-400 text-xs font-normal">~ {convertToBs(parseFloat(formData.costPrice) || 0)} Bs</span>
          </label>
        ...
        <div className="space-y-1 relative">
          <label className="text-sm font-medium flex justify-between text-slate-700 dark:text-slate-300">
            ...
            <span className="text-slate-400 text-xs font-normal">~ {convertToBs(parseFloat(formData.salePrice) || 0)} Bs</span>
          </label>
          <div className="absolute left-3 top-[34px] text-slate-400">$</div>
